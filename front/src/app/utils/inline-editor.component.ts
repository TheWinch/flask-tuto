...
@Component({
    selector: 'osc-inline-edit',
    template: `<div *ngIf="editing">
  <label *ngIf="label" class="col-form-label">{{label}}</label>
  <div class="input-group">
      <input #inputEditorControl class="form-control" [class.is-invalid]="inputReqflag"
          [required]="required" [id]="id" [(ngModel)]="value" type="text"
          [placeholder]="placeholder" [maxlength]="stringlength" />
...
<div *ngIf="!editing">
  <div class="form-group">
      <label *ngIf="label" class="col-form-label">{{label}}</label>
      <div *ngIf="IsInputTextEmpty()" (click)="edit(value)" (focus)="edit(value);" tabindex="0"
          class="inline-edit-empty">
          {{placeholder}}&nbsp;
      </div>
      <div *ngIf="!IsInputTextEmpty()" (click)="edit(value)" (focus)="edit(value);" tabindex="0"
          [ngClass]="disabled == 'true' ? 'inline-no-edit' : 'inline-edit'">{{value}}&nbsp;</div>
  </div>
</div>` ,
    styles: [
...
export class InlineEditorComponent implements ControlValueAccessor, OnInit {
    ...
    onSaveInput() {
        if (this.required == 'true') {
            if (this.inputEditorControl.nativeElement.value == null ||
